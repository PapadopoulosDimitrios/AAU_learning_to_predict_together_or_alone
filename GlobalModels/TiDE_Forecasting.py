...
def rolling_forecasting_validation_predictions(
    ...
    for window_start in val_starts:
        ...
        model = TiDE(
            ...
            temporal_decoder_dim=model_params["temporal_decoder_dim"],
            dropout=model_params["dropout"],
            layernorm=model_params["layernorm"],
            num_encoder_layers=model_params["num_encoder_layers"],
            num_decoder_layers=model_params["num_decoder_layers"],
            temporal_width=model_params["temporal_width"],
...
def compute_average_rmse_per_cluster(val_df, val_preds_df, pred_col="TiDE"):
...
def objective(trial):
    ...
    model_params = {
        ...
        "temporal_decoder_dim": trial.suggest_categorical("temporal_decoder_dim", [64, 128, 256]),
        "dropout": trial.suggest_float("dropout", 0.0, 0.3),
        "layernorm": trial.suggest_categorical("layernorm", [True, False]),
        "num_encoder_layers": trial.suggest_int("num_encoder_layers", 1, 3, step=1),
        "num_decoder_layers": trial.suggest_int("num_decoder_layers", 1, 3, step=1),
        "temporal_width": trial.suggest_categorical("temporal_width", [2, 4, 8]),
    ...
    for day_name in days:
        ...
        for cluster in range(clusters):
            ...
            final_TiDE = TiDE(
                ...
                temporal_decoder_dim=best_params["temporal_decoder_dim"],
                dropout=best_params["dropout"],
                layernorm=best_params["layernorm"],
                num_encoder_layers=best_params["num_encoder_layers"],
                num_decoder_layers=best_params["num_decoder_layers"],
                temporal_width=best_params["temporal_width"],
        ...
        # --------------------------------------------------
        save_dir = pathlib.Path(project_path) / "Outputs" / "Global models" / "TiDE"
        save_dir.mkdir(parents=True, exist_ok=True)

        save_path = save_dir / f"prediction_TiDE_{day_name}_{country}.csv"
...
data["Global"]["TiDE"] = total_seconds
