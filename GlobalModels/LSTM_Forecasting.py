...
def compute_average_rmse_per_cluster(val_df, val_preds_df, pred_col="LSTM"):
...
def objective(trial):
    ...
    model_params = {
        ...
        "context_size": trial.suggest_int("context_size", 5, 15, step=1),
...
for country in countries:
    ...
    for day_name in days:
        ...
        # --------------------------------------------------
        save_dir = pathlib.Path(project_path) / "Outputs" / "Global models" / "LSTM"
        save_dir.mkdir(parents=True, exist_ok=True)

        save_path = save_dir / f"prediction_LSTM_{day_name}_{country}.csv"
...
data["Global"]["LSTM"] = total_seconds
